...
VOL_MAX=$(pactl list sinks | grep "^[[:space:]]Base Volume:" | sed -E -e 's/^[^0-9]*([0-9]*).*$/\1/')
VOL_CUR=$(pactl list sinks | grep "^[[:space:]]Volume:"      | sed -E -e 's/^[^0-9]*([0-9]*).*$/\1/')
IS_MUTE=$(pactl list sinks | grep "^[[:space:]]Mute:"        | sed -E -e 's/.*(yes|no).*/\1/')
...
I_L=0
I_H=$VOL_STP
while [ ! $I_L -le $VOL_CUR -o ! $VOL_CUR -lt $I_H ]; do
    I_L=$(($I_L + $VOL_STP))
    I_H=$(($I_H + $VOL_STP))
done
