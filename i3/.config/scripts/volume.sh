...
VOL_PCT=$(echo "$SINK" | grep "^[[:space:]]volume:"      | sed -E -e 's/^.*\/ *([0-9]*\%).*$/\1/')
...
function print_help {
	echo "Usage: ./volume.sh <-u|-d|-t|-p|-h>"
	echo "Changes the system volume level (default pulse audio sink)."
	...
	echo "	-p | --print  Prints the system volume."
